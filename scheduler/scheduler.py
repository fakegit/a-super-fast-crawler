...
custom_logger = LogManager(logFile='scheduler.log', logLevel=5).logger
...
class Task(object):
    ...
    def run(self):
        custom_logger.debug(
            "Running %s task, scheduled at: %s" %
            (self.name, self.scheduled_time,))
        if self.halt_flag.isSet():
            return
        try:
            self.func()
        except:
            ...
            custom_logger.debug("Scheduled next run of %s for: %s" % (self.name, self.scheduled_time,))
...
class Scheduler(Thread):
    ...
    def drop(self, task_receipt):
        ...
        try:
            ...
            custom_logger.error('Invalid task receipt: %s' % (task_receipt,))
        self.tasks_lock.release()
    ...
    def run(self):
        while 1:
            ...
            if receipt != None:
                ...
                custom_logger.debug("Next task is %s in %s seconds" % (self.tasks[receipt].name, time_to_wait,))
                self.halt_flag.wait(secs_to_wait)
                try:
                    try:
                        self.tasks_lock.acquire()
                        task = self.tasks[receipt]
                        custom_logger.debug("Running %s..." % (task.name,))
                        task.run()
                    finally:
                        self.tasks_lock.release()
                except Exception, e:
                    custom_logger.exception(e)
                    custom_logger.debug( self.tasks )
            else:
                self.nonempty.wait()
    ...
    def __call__(self):
        ...
        while not is_success and try_count < self.num_tries:
            ...
            try:
                ...
                custom_logger.exception(e)
                custom_logger.error("Task failed on try #%s" % (try_count,))
                continue

        if is_success:
            custom_logger.info("Task %s was run successfully." % (self.func.__name__,))
        else:
            custom_logger.error("Success was not achieved!")
...
class RunOnce(object):
    ...
    def __call__(self):
        try:
            ...
            custom_logger.exception(e)
            custom_logger.error("Task failed: %s" % (e))
